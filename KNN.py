...
def experiment(file_name):
    ...
    k_params_list = [5, 10, 20, 25, 30, 40, 50, 100, 120, 130, 150, 175, 200, 250]
    successes_rate = []
...
class KNN:
    ...
    def find_KNN_examples(self, example):
        ...
        for i in range(len(self.data)):
            e_distance = euclidean_distance(example, self.data[i])
            #print(e_distance)
    ...
    def test(self, tester):
        ...
        for i in range(len(tester)):
            if tester[i][0] is self.classify(tester[i]):
                right += 1
        print(right *100 / size)
        return right*100 / size
